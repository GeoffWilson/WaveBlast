            ...
            switch (spriteName) {
                case "turret":
                    for (int i = 1; i < 3; i++) {
                        ...
                        if (cache.contains(frameName)) frames.add(cache.getFromCache(frameName));
                            ...
                            InputStream stream = SpriteAnimation.class.getResourceAsStream(frameName);
                            BufferedImage newFrame = ImageIO.read(stream);
                            frames.add(newFrame);
                ...
                default:
                    if (cache.contains(spriteName)) frames.add(cache.getFromCache(spriteName));
                        ...
                        InputStream stream = SpriteAnimation.class.getResourceAsStream("/" + spriteName);
                        BufferedImage newFrame = ImageIO.read(stream);
                        frames.add(newFrame);
